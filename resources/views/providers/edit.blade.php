<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Edit Provider / NAP</h1>
        </div>
    </x-slot>
        ...
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('providers.update', $provider) }}">
                ...
                <select name="type" required style="{{ $inp }}">
                    <option value="Provider" {{ old('type', $provider->type) == 'Provider' ? 'selected' : '' }}>Provider</option>
                    <option value="NAP" {{ old('type', $provider->type) == 'NAP' ? 'selected' : '' }}>NAP</option>
                </select>
                ...
                <input type="text" name="name" value="{{ old('name', $provider->name) }}" required style="{{ $inp }}">
