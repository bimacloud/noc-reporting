<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Edit Upstream: {{ $upstream->peer_name }}</h1>
        </div>
    </x-slot>
    <div style="max-width:540px;">
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('upstreams.update', $upstream) }}">
                @csrf @method('PUT')
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
                    <div>
                        ...
                        <input type="text" name="capacity" value="{{ old('capacity', $upstream->capacity) }}" style="{{ $inp }}">
                    </div>
                    <div>
                        ...
                        <select name="location_id" style="{{ $inp }}">
                            ...
                            @foreach($locations as $loc)
                                <option value="{{ $loc->id }}" {{ old('location_id', $upstream->location_id)==$loc->id?'selected':'' }}>{{ $loc->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
