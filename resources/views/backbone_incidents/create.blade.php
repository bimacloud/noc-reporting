<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Log Backbone Incident</h1>
        </div>
    </x-slot>
    <div style="max-width:640px;">
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('backbone_incidents.store') }}">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="backbone_link_id" required style="{{ $inp }}">
                            ...
                            @foreach($backboneLinks as $link)
                                <option value="{{ $link->id }}" {{ old('backbone_link_id')==$link->id?'selected':'' }}>{{ $link->node_a }} → {{ $link->node_b }}</option>
                            @endforeach
                        </select>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:16px;">
                    <div>
                        ...
                        <input type="number" name="latency" value="{{ old('latency') }}" min="0" step="0.01" style="{{ $inp }}">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="down_status" style="{{ $inp }}">
                            <option value="0" {{ old('down_status')==='0'?'selected':'' }}>Up</option>
                            <option value="1" {{ old('down_status')==='1'?'selected':'' }}>Down</option>
                        </select>
                    </div>
                </div>
