    ...
    <div style="max-width:640px;">
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <div style="margin-bottom:20px;padding-bottom:15px;border-bottom:1px solid #f3f4f6;">
                <p style="font-size:0.875rem;color:#374151;margin:0 0 5px 0;"><strong>Link:</strong> {{ $incident->backboneLink->node_a ?? 'Unknown' }} → {{ $incident->backboneLink->node_b ?? 'Unknown' }}</p>
            ...
            <form method="POST" action="{{ route('backbone_incidents.resolve', $incident) }}">
                @csrf @method('PUT')
