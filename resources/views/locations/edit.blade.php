<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Edit Location: {{ $location->name }}</h1>
        </div>
    </x-slot>
        ...
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('locations.update', $location) }}">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <textarea name="address" rows="2" style="width:100%;border:1px solid #d1d5db;border-radius:6px;padding:8px 12px;font-size:.875rem;color:#111827;box-sizing:border-box;">{{ old('address', $location->address) }}</textarea>
                ...
                <div style="display:flex;gap:10px;">
                    <button type="submit" style="padding:9px 20px;background:#2563eb;color:#fff;border:none;border-radius:6px;font-size:.875rem;font-weight:500;cursor:pointer;">Update Location</button>
