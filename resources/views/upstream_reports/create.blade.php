<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Add Upstream Report</h1>
        </div>
    </x-slot>
    <div style="max-width:640px;">
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('upstream_reports.store') }}">
                @csrf
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
                    <div>
                        ...
                        <select name="upstream_id" required style="{{ $inp }}">
                            <option value="">— Select Upstream —</option>
                            @foreach($upstreams as $up)
                                <option value="{{ $up->id }}" {{ old('upstream_id')==$up->id?'selected':'' }}>{{ $up->peer_name }}</option>
                            @endforeach
                        </select>
                        @error('upstream_id') <p style="color:#dc2626;font-size:.75rem;margin-top:4px;">{{ $message }}</p> @enderror
                    </div>
                    <div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:16px;">
                    @foreach(['status_l1' => 'L1 Status', 'status_l2' => 'L2 Status', 'status_l3' => 'L3 Status'] as $field => $label)
                    <div>
                        ...
                        <select name="{{ $field }}" style="{{ $inp }}">
                            @foreach(['up','down','degraded','na'] as $s)
                                <option value="{{ $s }}" {{ old($field, 'up')===$s?'selected':'' }}>{{ ucfirst($s) }}</option>
                            @endforeach
                        </select>
                    </div>
                    @endforeach
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
                    ...
                    <div>
                        ...
                        <input type="text" name="notes" value="{{ old('notes') }}" style="{{ $inp }}">
                    </div>
                </div>
