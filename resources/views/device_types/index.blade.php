<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;justify-content:space-between;">
            <div>
                <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Device Types</h1>
                <p style="font-size:.875rem;color:#6b7280;margin:4px 0 0;">Manage categories for network devices.</p>
            </div>
            @if(Auth::user()->isAdmin() || Auth::user()->isNoc())
            <a href="{{ route('device_types.create') }}" style="padding:9px 16px;background:#2563eb;color:#fff;border-radius:6px;font-size:.875rem;font-weight:500;text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
    ...
    <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;box-shadow:0 1px 3px rgba(0,0,0,.06);overflow:hidden;max-width:800px;">
        <table style="width:100%;border-collapse:collapse;text-align:left;">
            ...
            <tbody style="divide-y:1px solid #e5e7eb;">
                ...
                <tr style="hover:bg-slate-50">
                    ...
                    <td style="padding:10px 16px;text-align:right;">
                        <div style="display:flex;justify-content:flex-end;gap:8px;">
                            @if(Auth::user()->isAdmin() || Auth::user()->isNoc())
                            <a href="{{ route('device_types.edit', $type) }}" style="color:#2563eb;"><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                            <form action="{{ route('device_types.destroy', $type) }}" method="POST" onsubmit="return confirm('Are you sure? This might affect existing devices.');" style="display:inline;">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="3" style="padding:24px;text-align:center;color:#6b7280;font-size:.875rem;">No device types found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
