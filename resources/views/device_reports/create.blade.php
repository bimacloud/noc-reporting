<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <h1 style="font-size:1.125rem;font-weight:700;color:#111827;margin:0;">Add Device Report</h1>
        </div>
    </x-slot>
    <div style="max-width:640px;">
        <div style="background:#fff;border-radius:10px;border:1px solid #e5e7eb;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <form method="POST" action="{{ route('device_reports.store') }}">
                @csrf
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
                    <div>
                        ...
                        <select name="device_id" required style="{{ $inp }}">
                            <option value="">— Select Device —</option>
                            @foreach($devices as $d)
                                <option value="{{ $d->id }}" {{ old('device_id')==$d->id?'selected':'' }}>{{ $d->name }}</option>
                            @endforeach
                        </select>
                        @error('device_id') <p style="color:#dc2626;font-size:.75rem;margin-top:4px;">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label style="display:block;font-size:.875rem;font-weight:500;color:#374151;margin-bottom:6px;">Month *</label>
                        <input type="month" name="month" value="{{ old('month', date('Y-m')) }}" required style="{{ $inp }}">
                        @error('month') <p style="color:#dc2626;font-size:.75rem;margin-top:4px;">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:16px;">
                    @foreach(['physical_status' => 'Physical Status', 'psu_status' => 'PSU Status', 'fan_status' => 'Fan Status'] as $field => $label)
                    <div>
                        ...
                        <select name="{{ $field }}" style="{{ $inp }}">
                            @foreach(['good','bad','na'] as $s)
                                <option value="{{ $s }}" {{ old($field, 'good')===$s?'selected':'' }}>{{ ucfirst($s) }}</option>
                            @endforeach
                        </select>
                    </div>
                    @endforeach
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
                    <div>
                        <label style="display:block;font-size:.875rem;font-weight:500;color:#374151;margin-bottom:6px;">Downtime (minutes)</label>
                        <input type="number" name="duration_downtime" value="{{ old('duration_downtime', 0) }}" min="0" style="{{ $inp }}">
                    </div>
                    <div>
                        <label style="display:block;font-size:.875rem;font-weight:500;color:#374151;margin-bottom:6px;">Downtime Cause</label>
                        <input type="text" name="downtime_cause" value="{{ old('downtime_cause') }}" style="{{ $inp }}" placeholder="Optional">
                    </div>
                </div>
                ...
                <div style="display:flex;gap:10px;">
                    <button type="submit" style="padding:9px 20px;background:#2563eb;color:#fff;border:none;border-radius:6px;font-size:.875rem;font-weight:500;cursor:pointer;">Save Report</button>
